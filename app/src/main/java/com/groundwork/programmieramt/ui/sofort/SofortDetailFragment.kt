...
class SofortDetailFragment : Fragment() {
    ...
    private fun updateHeader() {
        val parts = buildList {
            if (kategorie.isNotBlank()) add(kategorie)
    ...
    private fun showMetadataDialog() {
        ...
        val etKategorie = EditText(requireContext()).apply {
            hint = getString(R.string.field_kategorie)
            setText(kategorie)
            inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_FLAG_CAP_SENTENCES
            layoutParams = LinearLayout.LayoutParams(MATCH_PARENT, WRAP_CONTENT).apply { topMargin = p8 }
        }
        container.addView(etKategorie)
        ...
        val dialog = AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("OK") { _, _ ->
                kategorie = etKategorie.text.toString().trim()
